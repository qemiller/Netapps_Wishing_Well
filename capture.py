...
connection = pika.BlockingConnection(pika.ConnectionParameters('172.30.71.101', 5672, '/', credentials))
...
class listener(StreamListener):
	def on_data(self, data):
		...
		if token_tweet["type"] == 'p':
			...
			a,b,sendbackbody=channel.basic_get(queue=token_tweet['subject']) #change on sunday to consume from capture
			...
			print("tweet consumed by capture pi: ", consumed_tweet['message']) #'print out to monitor the consumed tweet'
		#check point 4 
